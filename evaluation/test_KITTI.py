...
if __name__ == '__main__':
    ...
    parser.add_argument('--chosen_snapshot', default='VBReg_KITTI_release', type=str, help='snapshot dir')
    ...
    from models.VBReg import VBReg as Model
    model = Model(config)
    config.tag = f'snapshot/{args.chosen_snapshot}/models/model_best.pkl'
    miss = model.load_state_dict(torch.load(config.tag), strict=False)
    print(miss)
    model.eval()
