import os, pickle, glob, torch.utils.data as data
...
class ThreeDMatchTrainVal(data.Dataset):
    def __init__(self,
        ...
        assert descriptor in ['fpfh', 'fcgf', 'predator']
        self.in_dim = in_dim
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train' and np.mean(labels) > 0.5:
            ...
            soft_labels = np.concatenate([soft_labels, np.ones(num_outliers) * 10.], axis=0)
        else:
        ...
        if self.in_dim == 3:
            ...
            corr_pos = np.concatenate([input_src_keypts, input_tgt_keypts, input_src_keypts - input_tgt_keypts],
                                      axis=-1)
        elif self.in_dim == 70:
            corr_pos = np.concatenate([input_src_keypts, input_tgt_keypts], axis=-1)
            corr_pos = corr_pos - corr_pos.mean(0)
        ...
        soft_labels = np.clip(1.0 - soft_labels ** 2 / self.inlier_threshold ** 2, a_min=0., a_max=1.0)
        return corr_pos.astype(np.float32), \
               ...
               np.concatenate([labels.astype(np.float32)[None], soft_labels[None]]).T.astype(np.float32)
...
class ThreeDMatchTest(data.Dataset):
    def __init__(self,
        ...
        for scene in self.scene_list:
            # scene_path = f'{self.root}/test_db/fragments/{scene}'
...
class ThreeDLOMatchTest(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.descriptor == 'fcgf':
            ...
            src_data = np.load(f"{self.root}/test_db/fragments/{scene}/cloud_bin_{src_id}_fpfh_orient0.npz")
            tgt_data = np.load(f"{self.root}/test_db/fragments/{scene}/cloud_bin_{tgt_id}_fpfh_orient0.npz")
            src_keypts = src_data['xyz']
            tgt_keypts = tgt_data['xyz']
            src_features = src_data['feature']
            tgt_features = tgt_data['feature']
